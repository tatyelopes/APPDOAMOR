...
type Screen = 'home' | 'questions' | 'test' | 'result' | 'temperament' | 'temperamentResult' | 'profile' | 'private'
...
type Temperament = 'Colérico' | 'Sanguíneo' | 'Fleumático' | 'Melancólico'
const API = '/api'
...
export default function App() {
  ...
  const content = loading
    ...
    : !token || !user
      ...
      : !user.couple
        ? <Connect token={token} user={user} onConnected={setUser} onLogout={() => { localStorage.removeItem('entrenos-token'); setToken(''); setUser(null) }} />
        : <Experience token={token} user={user} onUser={setUser} onLogout={() => { localStorage.removeItem('entrenos-token'); setToken(''); setUser(null) }} />
...
function Experience({ token, user, onUser, onLogout }: { token: string; user: ApiUser; onUser: (user: ApiUser) => void; onLogout: () => void }) {
  ...
  const [temperamentAnswers, setTemperamentAnswers] = useState<Temperament[]>(() => JSON.parse(localStorage.getItem('app-do-amor-temperaments') || '[]'))
  ...
  function chooseTemperament(temperament: Temperament) {
    ...
    if (temperamentIndex === temperamentQuestions.length - 1) {
      localStorage.setItem('app-do-amor-temperaments', JSON.stringify(next))
      setStreak(value => value + 1)
      setScreen('temperamentResult')
    } else setTemperamentIndex(value => value + 1)
  }
  function startTemperament() {
    setTemperamentIndex(0); setTemperamentAnswers([]); setScreen('temperament')
  }
  ...
  return <div className="app-shell">
    <header className="topbar">
      <button className="brand" onClick={() => setScreen('home')}><span className="brand-mark">♥</span><span className="brand-name"><b>APP-do-amor</b><small>conexão que vira memória</small></span></button>
