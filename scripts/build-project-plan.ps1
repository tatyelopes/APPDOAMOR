...
40;Infraestrutura;DevOps;Configurar pipeline de CI;Pipeline funcional;Alta;39;DevOps/Plataforma;Concluído;10;17/09/2026;17/09/2026;Workflow GitHub Actions em .github/workflows/ci.yml com Ubuntu 24.04 Node 24.20.0 cache npm instalação por lockfile permissões somente leitura concorrência timeout e gates de lint formato build OpenAPI ameaças LGPD feedback produção matriz móvel e planilha — documentação em docs/INTEGRACAO-CONTINUA.md — execução remota 1 aprovada em https://github.com/tatyelopes/APPDOAMOR/actions/runs/35267092012
41;Infraestrutura;DevOps;Provisionar banco PostgreSQL;Banco por ambiente;Crítica;34,38;Engenharia Backend;Em andamento;10;16/09/2026;;PostgreSQL gratuito temporário do feedback ativo no Render — em 17/09/2026 foram adicionados PostgreSQL 18 local via Compose executor transacional com lock e checksum duas migrações versionadas para 12 tabelas de domínio mais schema_migrations validação real aprovada em https://github.com/tatyelopes/APPDOAMOR/actions/runs/35269876662 e bootstrap automático da API aprovado no CI em https://github.com/tatyelopes/APPDOAMOR/actions/runs/35288177777 — após novo deploy a migração automática e a exportação protegida foram confirmadas ao vivo com base vazia — ainda integrar cadastro casal sessões respostas e analytics migrar dados legados com reconciliação e provisionar homologação e produção definitivas isoladas
42;Infraestrutura;DevOps;Configurar gestão segura de segredos;Cofre de segredos;Crítica;38;DevOps/Plataforma;Em andamento;6;17/09/2026;;Baseline gratuita implementada em docs/GESTAO-DE-SEGREDOS.md com inventário matriz por ambiente geração local criptograficamente aleatória sem exibição validação fail-closed em runtime scanner de arquivos rastreados staged e histórico hook pre-commit gate aprovado em https://github.com/tatyelopes/APPDOAMOR/actions/runs/35272382976 modelo de rotação e injeção atual do Render — token de exportação do MPV rotacionado em 17/09/2026 após exposição acidental e salvo localmente sem exibição — pendentes cofres e identidades definitivos de homologação e produção e responsáveis nominais
43;Infraestrutura;Observabilidade;Configurar logs métricas e alertas;Observabilidade básica;Alta;38;DevOps/Plataforma;Concluído;12;17/09/2026;17/09/2026;Logs JSON com ambiente versão request ID rota normalizada status e duração sem corpo ou mensagem interna — erros 500 genéricos — sondas separadas de vida e prontidão com consulta real ao PostgreSQL — métricas de requisições erros prontidão latência memória e uptime protegidas — monitor gratuito do GitHub a cada 15 minutos com três tentativas abertura de issue única e encerramento após recuperação — teste com canário privado e documentação em docs/OBSERVABILIDADE.md — histórico durável thresholds por ambiente e alertas de backup seguem nas tarefas 123 118 e 93
