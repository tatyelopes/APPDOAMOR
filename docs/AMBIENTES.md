...
A publicação controlada não encerra os gates de produção do restante do aplicativo. HTTPS, banco, segredo restrito e exportação foram confirmados. A passagem física no Chrome de 16/09/2026 cobriu a versão anterior; a ampliação com baralho visual e amostra autoral, feita em 17/09/2026, exige nova passagem física na tarefa 142. A Product Owner adiou o Safari, que continua sem cobertura física.
...
A observabilidade básica está implementada conforme o [guia operacional](OBSERVABILIDADE.md): logs JSON identificam ambiente, versão, request ID, rota normalizada, status e duração sem respostas íntimas, tokens, convites, corpos ou e-mails; `/api/health` mede vida e `/api/ready` consulta armazenamento e schema; métricas em memória ficam protegidas em `/api/ops/metrics`; e um workflow gratuito tenta a prontidão três vezes a cada 15 minutos, abre uma issue única na falha e a encerra após recuperação. Histórico durável, thresholds de erro e capacidade entram no provisionamento da tarefa 123. Alerta de backup depende da implementação e do ensaio das tarefas 118 e 93.
