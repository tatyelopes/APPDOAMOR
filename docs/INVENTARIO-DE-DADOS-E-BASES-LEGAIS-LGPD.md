...
O inventário parte do código e dos documentos existentes em 05/09/2026 e foi atualizado em 17/09/2026 com o feedback centralizado e as migrações PostgreSQL. O ambiente remoto controlado usa PostgreSQL para feedback; os fluxos antigos do protótipo ainda usam JSON no servidor, guardam respostas e eventos sem criptografia de campo, mantêm tokens de sessão em texto no servidor e no `localStorage`, e persistem localmente respostas do teste de temperamento. O schema alvo agora é executável e prevê hashes de tokens, isolamento por casal e respostas bilaterais, mas esses repositórios ainda não estão integrados. O inventário distingue esse estado atual do tratamento planejado; ele não declara conformidade da implementação.
...
| DG-09 | Respostas e resultados da amostra de preferências afetivas | Pessoal inferido e **potencialmente sensível** | Titular e algoritmo local | Implementado no MPV somente em memória do navegador; persistência futura só mediante decisão |
...
| OP-10 | Amostra e inferência de preferências afetivas; DG-09; usuário | Gerar resultado educativo solicitado | Art. 11, I, quando respostas ou inferência puderem revelar dado sensível; Art. 7º, I, nos demais casos | Próprio titular por padrão; parceiro somente por compartilhamento granular | Processar em memória por padrão; persistir só com consentimento separado; RT-07 | MPV executa somente em memória e informa caráter autoral e não diagnóstico; revisão jurídica e consentimento específico continuam pendentes para persistência futura |
