...
export function buildMetrics(db, now = Date.now()) {
  ...
  const mutualAll = uniqueCouples(
    events.filter((event) => event.name === 'mutual_experience_completed'),
  )
  const mutual7 = uniqueCouples(
    since(7).filter((event) => event.name === 'mutual_experience_completed'),
  )
  ...
  function retention(days) {
    ...
    const retained = eligible.filter((couple) => {
      ...
      return events.some(
        (event) =>
          event.coupleId === couple.id &&
          meaningfulEvents.has(event.name) &&
          eventDate(event) >= created + days * day,
      )
    })
    return {
      eligible: eligible.length,
      retained: retained.length,
      rate: percent(retained.length, eligible.length),
    }
  }
  ...
  const weeks = Array.from({ length: 8 }, (_, reverseIndex) => {
    ...
    return {
      ...
      activeCouples: uniqueCouples(weekEvents.filter((event) => meaningfulEvents.has(event.name)))
        .size,
      mutualExperiences: weekEvents.filter((event) => event.name === 'mutual_experience_completed')
        .length,
    }
  })
  ...
  return {
    ...
    activation: {
      ...
      pairedToFirstExperienceRate: percent(
        [...mutualAll].filter((id) => pairedIds.has(id)).length,
    ...
    engagement: {
      ...
      mutualExperiences7d: since(7).filter((event) => event.name === 'mutual_experience_completed')
        .length,
      reciprocityRate7d: percent(
        mutual7.size,
        uniqueCouples(since(7).filter((event) => event.name === 'answer_submitted')).size,
      ),
    },
