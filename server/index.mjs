...
import { hashPassword as passwordHash, matchesPassword as passwordMatches } from './src/shared/security.js'
...
function publicUser(user, db) {
  ...
  return { id: user.id, name: user.name, email: user.email, isAdmin: config.adminEmails.includes(user.email), couple: couple ? { id: couple.id, code: couple.code, anniversary: couple.anniversary, partner: partner ? { id: partner.id, name: partner.name } : null } : null }
}
...
const server = createServer(async (req, res) => {
  ...
  try {
    if (req.method === 'POST' && req.url === '/api/register') {
      ...
      const user = { id: randomUUID(), name: input.name.trim(), email, password: passwordHash(input.password), coupleId: null, createdAt: new Date().toISOString() }
      const token = randomBytes(32).toString('hex')
      db.users.push(user); db.sessions.push({ token, userId: user.id, expiresAt: Date.now() + 30 * 864e5 }); recordEvent(db, 'user_registered', user); saveDb(db)
    ...
    if (req.method === 'POST' && req.url === '/api/login') {
      ...
      const token = randomBytes(32).toString('hex'); db.sessions.push({ token, userId: user.id, expiresAt: Date.now() + 30 * 864e5 }); recordEvent(db, 'user_logged_in', user); saveDb(db)
    ...
    if (req.method === 'GET' && req.url === '/api/admin/metrics') {
      if (!config.adminEmails.includes(user.email)) return send(res, 403, { error: 'Acesso restrito à administração.' })
    ...
    if (req.method === 'POST' && req.url === '/api/events') {
      ...
      recordEvent(db, name, user, input.properties); saveDb(db)
    ...
    if (req.method === 'POST' && req.url === '/api/couples/create') {
      ...
      const couple = { id: randomUUID(), code: invite, anniversary: input.anniversary || '', members: [user.id], createdAt: new Date().toISOString() }
      user.coupleId = couple.id; db.couples.push(couple); recordEvent(db, 'couple_created', user); saveDb(db)
    ...
    if (req.method === 'POST' && req.url === '/api/couples/join') {
      ...
      couple.members.push(user.id); user.coupleId = couple.id; couple.pairedAt = new Date().toISOString(); recordEvent(db, 'couple_paired', user); saveDb(db)
    ...
    if (req.method === 'POST' && req.url?.startsWith('/api/answers/')) {
      ...
      const completedNow = !found && db.answers.filter(item => item.coupleId === user.coupleId && item.questionId === questionId).length === 2
      const alreadyCounted = db.analyticsEvents.some(event => event.name === 'mutual_experience_completed' && event.coupleId === user.coupleId && event.properties?.questionId === questionId)
      if (completedNow && !alreadyCounted) recordEvent(db, 'mutual_experience_completed', user, { questionId })
